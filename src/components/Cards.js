...
const Cards = () => {
    ...
    return (
        <div className='flex flex-wrap justify-between p-4 mt-2 max-w-[1240px] mx-auto'>
            {
                data.map((e, i) => {
                    return (
                        <div key={i} className='bg-neutral-800 font-bold shadow-lg p-2 hover:-translate-y-2 transition-all duration-500 cursor-pointer mt-6 '>
                            <img className='h-72' src={e.img} alt='img-movie' />
                            <h1><span className='text-gray-400 mr-2'>Name: </span>{e.name}</h1>
